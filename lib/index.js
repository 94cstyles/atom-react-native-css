...
module.exports = {
    config: {
        ...
        enableSass: {
            ...
            description: 'By default only supports css files, you need to use css file Enable this',
            type: 'boolean',
            "default": false,
            order: 12,
        },
    ...
    installNodeSass: function() {
        ...
        var dialog = atom.notifications.addInfo('Installing node-sass ...', {
            dismissable: true
        });
        exec((process.platform === 'win32' ? 'npm.cmd' : 'npm') + ' install node-sass', {
            ...
            if (error !== null) {
                this.nodeSassInstallState = -1;
                atom.notifications.addError('node-sass install failure', {
                    detail: error,
                    dismissable: true
                });
            } else {
                this.nodeSassInstallState = 0;
                atom.notifications.addSuccess('node-sass install success', {
                    detail: 'node-sass install success，You can now use sass edit React Style。',
                    dismissable: false
                });
            }
        });
