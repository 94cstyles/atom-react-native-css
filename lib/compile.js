...
var compile = {
    ...
    formatJSS: function(obj, property, value) {
        ...
        if (/^(padding|margin)$/.test(property)) {
            ...
            if (/^(normal|bold|[1-9]00)$/.test(value)) {
                ...
                obj[property] = value + '';
            } else if (/^['|"]?(bolder|lighter|initial|inherit|unset)['|"]?$/.test(value)) {
              switch (value) {
                case 'bolder':
                  obj[property] = '900';
                  atom.notifications.addInfo('`bolder` value set to `900`')
                  break;
                case 'lighter':
                  obj[property] = '200';
                  atom.notifications.addInfo('`lighter` value set to `200`')
                  break;
                default:
                  atom.notifications.addWarning('`' + value + '` value was not set')
              }
            } else {
    ...
    parseSCSSGlobally: function(filePath, outputFile, errorFunc) {
        ...
        var css = fs.readFileSync(tmp_file,'utf-8');
        fs.unlink(tmp_file);
    ...
    doParse: function (contents, filePath, outputFile, errorFunc) {
      try {
          var tmp_contents = contents.replace(/\r?\n|\r/g, '');
          contents = css.parse(tmp_contents);
      } catch (e) {
